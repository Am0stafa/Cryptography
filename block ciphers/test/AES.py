from KeyExpansion import AES_KeyExpansion
from keyEx import keyExpansion
import copy
import codecs


class AES:
	rounds_by_key_size = {16: 10, 24: 12, 32: 14}
	def __init__(self,key,mode):
		self.key = key
		self.mode = mode
		self.sbox = [
			 [0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76],
			 [0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0],
			 [0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15],
			 [0x04, 0xc7, 0x23, 0xc3, 0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27, 0xb2, 0x75],
			 [0x09, 0x83, 0x2c, 0x1a, 0x1b, 0x6e, 0x5a, 0xa0, 0x52, 0x3b, 0xd6, 0xb3, 0x29, 0xe3, 0x2f, 0x84],
			 [0x53, 0xd1, 0x00, 0xed, 0x20, 0xfc, 0xb1, 0x5b, 0x6a, 0xcb, 0xbe, 0x39, 0x4a, 0x4c, 0x58, 0xcf],
			 [0xd0, 0xef, 0xaa, 0xfb, 0x43, 0x4d, 0x33, 0x85, 0x45, 0xf9, 0x02, 0x7f, 0x50, 0x3c, 0x9f, 0xa8],
			 [0x51, 0xa3, 0x40, 0x8f, 0x92, 0x9d, 0x38, 0xf5, 0xbc, 0xb6, 0xda, 0x21, 0x10, 0xff, 0xf3, 0xd2],
			 [0xcd, 0x0c, 0x13, 0xec, 0x5f, 0x97, 0x44, 0x17, 0xc4, 0xa7, 0x7e, 0x3d, 0x64, 0x5d, 0x19, 0x73],
			 [0x60, 0x81, 0x4f, 0xdc, 0x22, 0x2a, 0x90, 0x88, 0x46, 0xee, 0xb8, 0x14, 0xde, 0x5e, 0x0b, 0xdb],
			 [0xe0, 0x32, 0x3a, 0x0a, 0x49, 0x06, 0x24, 0x5c, 0xc2, 0xd3, 0xac, 0x62, 0x91, 0x95, 0xe4, 0x79],
			 [0xe7, 0xc8, 0x37, 0x6d, 0x8d, 0xd5, 0x4e, 0xa9, 0x6c, 0x56, 0xf4, 0xea, 0x65, 0x7a, 0xae, 0x08],
			 [0xba, 0x78, 0x25, 0x2e, 0x1c, 0xa6, 0xb4, 0xc6, 0xe8, 0xdd, 0x74, 0x1f, 0x4b, 0xbd, 0x8b, 0x8a],
			 [0x70, 0x3e, 0xb5, 0x66, 0x48, 0x03, 0xf6, 0x0e, 0x61, 0x35, 0x57, 0xb9, 0x86, 0xc1, 0x1d, 0x9e],
			 [0xe1, 0xf8, 0x98, 0x11, 0x69, 0xd9, 0x8e, 0x94, 0x9b, 0x1e, 0x87, 0xe9, 0xce, 0x55, 0x28, 0xdf],
			 [0x8c, 0xa1, 0x89, 0x0d, 0xbf, 0xe6, 0x42, 0x68, 0x41, 0x99, 0x2d, 0x0f, 0xb0, 0x54, 0xbb, 0x16]
		]
		self.Rcon = [
			0x00000000, 0x01000000, 0x02000000,0x04000000, 0x08000000, 0x10000000,0x20000000, 0x40000000,0x80000000,0x1b000000, 0x36000000
		]
		self.n_rounds = AES.rounds_by_key_size[len(self.key)]
		self.keys = keyExpansion(self.key)
		self.decKeys = self.keys[::-1]
		self.inv_s_box = [
			[0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb],
			[0x7c, 0xe3, 0x39, 0x82, 0x9b, 0x2f, 0xff, 0x87, 0x34, 0x8e, 0x43, 0x44, 0xc4, 0xde, 0xe9, 0xcb],
			[0x54, 0x7b, 0x94, 0x32, 0xa6, 0xc2, 0x23, 0x3d, 0xee, 0x4c, 0x95, 0x0b, 0x42, 0xfa, 0xc3, 0x4e],
			[0x08, 0x2e, 0xa1, 0x66, 0x28, 0xd9, 0x24, 0xb2, 0x76, 0x5b, 0xa2, 0x49, 0x6d, 0x8b, 0xd1, 0x25],
			[0x72, 0xf8, 0xf6, 0x64, 0x86, 0x68, 0x98, 0x16, 0xd4, 0xa4, 0x5c, 0xcc, 0x5d, 0x65, 0xb6, 0x92],
			[0x6c, 0x70, 0x48, 0x50, 0xfd, 0xed, 0xb9, 0xda, 0x5e, 0x15, 0x46, 0x57, 0xa7, 0x8d, 0x9d, 0x84],
			[0x90, 0xd8, 0xab, 0x00, 0x8c, 0xbc, 0xd3, 0x0a, 0xf7, 0xe4, 0x58, 0x05, 0xb8, 0xb3, 0x45, 0x06],
			[0xd0, 0x2c, 0x1e, 0x8f, 0xca, 0x3f, 0x0f, 0x02, 0xc1, 0xaf, 0xbd, 0x03, 0x01, 0x13, 0x8a, 0x6b],
			[0x3a, 0x91, 0x11, 0x41, 0x4f, 0x67, 0xdc, 0xea, 0x97, 0xf2, 0xcf, 0xce, 0xf0, 0xb4, 0xe6, 0x73],
			[0x96, 0xac, 0x74, 0x22, 0xe7, 0xad, 0x35, 0x85, 0xe2, 0xf9, 0x37, 0xe8, 0x1c, 0x75, 0xdf, 0x6e],
			[0x47, 0xf1, 0x1a, 0x71, 0x1d, 0x29, 0xc5, 0x89, 0x6f, 0xb7, 0x62, 0x0e, 0xaa, 0x18, 0xbe, 0x1b],
			[0xfc, 0x56, 0x3e, 0x4b, 0xc6, 0xd2, 0x79, 0x20, 0x9a, 0xdb, 0xc0, 0xfe, 0x78, 0xcd, 0x5a, 0xf4],
			[0x1f, 0xdd, 0xa8, 0x33, 0x88, 0x07, 0xc7, 0x31, 0xb1, 0x12, 0x10, 0x59, 0x27, 0x80, 0xec, 0x5f],
			[0x60, 0x51, 0x7f, 0xa9, 0x19, 0xb5, 0x4a, 0x0d, 0x2d, 0xe5, 0x7a, 0x9f, 0x93, 0xc9, 0x9c, 0xef],
			[0xa0, 0xe0, 0x3b, 0x4d, 0xae, 0x2a, 0xf5, 0xb0, 0xc8, 0xeb, 0xbb, 0x3c, 0x83, 0x53, 0x99, 0x61],
			[0x17, 0x2b, 0x04, 0x7e, 0xba, 0x77, 0xd6, 0x26, 0xe1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0c, 0x7d]
			]
			
	
	
	# method to check if a cell length is 1 add 0 to the front in a two dimensional array
	def check_cell(self, list):
		for i in range(len(list)):
			for j in range(len(list[i])):
				if len(list[i][j]) == 1:
					list[i][j] = "0" + list[i][j]
		return list
	
	
	def shift_rows(self,s):
		rot = copy.deepcopy(s)
		rot[1][0], rot[1][1], rot[1][2], rot[1][3] = rot[1][1], rot[1][2], rot[1][3], rot[1][0]
		
		rot[2][0], rot[2][1], rot[2][2], rot[2][3] = rot[2][2], rot[2][3], rot[2][0], rot[2][1]
		
		rot[3][0], rot[3][1], rot[3][2], rot[3][3] = rot[3][3], rot[3][0], rot[3][1], rot[3][2]
		return rot

	def inv_shift_rows(self,s):
		rot = copy.deepcopy(s)
		rot[0][1], rot[1][1], rot[2][1], rot[3][1] = rot[3][1], rot[0][1], rot[1][1], rot[2][1]
		rot[0][2], rot[1][2], rot[2][2], rot[3][2] = rot[2][2], rot[3][2], rot[0][2], rot[1][2]
		rot[0][3], rot[1][3], rot[2][3], rot[3][3] = rot[1][3], rot[2][3], rot[3][3], rot[0][3]
		return rot
	
	def stateMatrix(self,m):
		str = (m).encode('utf-8')
		hexa = str.hex()
		stateMatrix = [[0 for x in range(4)] for y in range(4)]
		pair = []
		two = 1
		for i in range(0,16):
			pair.append(hexa[i*2]+hexa[two])
			two +=2
			
		
		for i in range(4):
			for j in range(4):
				stateMatrix[j][i] = pair[i*4+j]

		return stateMatrix
				
	def addRoundKey(self,stateMatrix,key):
		newMatrix = [[0 for x in range(4)] for y in range(4)]
		for i in range(4):
			for j in range(4):
				newMatrix[i][j] = self.xor(stateMatrix[i][j],key[i][j])  # type: ignore
		
		return newMatrix
				

	# function to xor two hexadecimal numbers in a string formate
	def xor(self,a, b):
		return hex(int(a, 16) ^ int(b, 16))[2:]
	
	def SubstitutionBytes(self,stateMatrix):
		afterSbox = [[0 for x in range(4)] for y in range(4)]
		for i in range(len(stateMatrix)):
			for j in range(len(stateMatrix[i])):
				a = stateMatrix[i][j][0]
				if not a.isdigit():
					a = ord(a) - 87
					
				b = stateMatrix[i][j][1]
				if not b.isdigit():
					b = ord(b) -87

				afterSbox[i][j] = (hex(self.sbox[int(a)][int(b)])[2:])    # type: ignore
		
		return afterSbox
		
	def inv_SubstitutionBytes(self,stateMatrix):
		afterSbox = [[0 for x in range(4)] for y in range(4)]
		for i in range(len(stateMatrix)):
			for j in range(len(stateMatrix[i])):
				a = stateMatrix[i][j][0]
				if not a.isdigit():
					a = ord(a) - 87
					
				b = stateMatrix[i][j][1]
				if not b.isdigit():
					b = ord(b) -87

				afterSbox[i][j] = (hex(self.inv_s_box[int(a)][int(b)])[2:])      # type: ignore
		
		return afterSbox
		
	
	def mixSingleColumn(self,a,l=0):
		xtime = lambda a: (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)
		
		if l == 0:
			for i in range(len(a)):
				a[i] = (int(a[i],16))
			
		t = a[0] ^ a[1] ^ a[2] ^ a[3]
		u = a[0]
		a[0] ^= t ^ xtime(a[0] ^ a[1])
		a[1] ^= t ^ xtime(a[1] ^ a[2])
		a[2] ^= t ^ xtime(a[2] ^ a[3])
		a[3] ^= t ^ xtime(a[3] ^ u)
	
	def mixColumns(self,s,l=0):
		mix = copy.deepcopy(s)
		for i in range(4):
			self.mixSingleColumn(mix[i],l)
		
		for i in range(len(mix)):
			for j in range(len(mix[i])):
				mix[i][j] = hex(mix[i][j])[2:]		
			
		return mix
		
		
	def inv_mix_columns(self,s):
		s = copy.deepcopy(s)
		
		for i in range(len(s)):
			for j in range(len(s[i])):
				s[i][j] = (int(s[i][j],16))

		xtime = lambda a: (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)
		for i in range(4):
			u = xtime(xtime(s[i][0] ^ s[i][2]))
			v = xtime(xtime(s[i][1] ^ s[i][3]))
			s[i][0] ^= u
			s[i][1] ^= v
			s[i][2] ^= u
			s[i][3] ^= v
	
		return self.mixColumns(s,1)

	def encryptedMsg(self,message):
		
		stateMatrix = self.stateMatrix(message)
		
		initialTransformation = self.addRoundKey(stateMatrix,self.keys[0])
		newStateMatrix = self.check_cell(initialTransformation)
		

		round9 = self.recEncrypt(newStateMatrix,1)
		
		
		subBytes10 = self.check_cell(self.SubstitutionBytes(round9))
		shiftRows10 = self.check_cell(self.shift_rows(subBytes10))
		round10 = self.check_cell(self.addRoundKey(shiftRows10,self.keys[10]))
		
		word = ''
		for i in range(len(round10)):
			word += ''.join(round10[i])  # type: ignore
		
		my_string = word
		my_string_bytes = bytes(my_string, encoding='latin-1')
		
		binary_string = codecs.decode(my_string_bytes, "hex")
		print(f"encrypt: {str(binary_string, 'latin-1')}")
		return round10
		
	def decryptMessage(self,stateMatrix):
		initialTransformation = self.addRoundKey(stateMatrix,self.decKeys[0])
		shiftRows = self.check_cell(self.inv_shift_rows(initialTransformation))
		invSub = self.check_cell(self.inv_SubstitutionBytes(shiftRows))

		round9 = self.recDecrypt(invSub,1)
		
		round10 = self.check_cell(self.addRoundKey(round9,self.decKeys[10]))
		
		dec = [[0 for x in range(4)] for y in range(4)]
		for i in range(len(round10)):
			for j in range(len(round10[i])):
				dec[j][i] = round10[i][j]
		
		word = ''
		for i in range(len(dec)):
			word += ''.join(dec[i])  # type: ignore
				

		my_string = word
		my_string_bytes = bytes(my_string, encoding='latin-1')
		
		binary_string = codecs.decode(my_string_bytes, "hex")
		print(f"decrypt: {str(binary_string, 'latin-1')}")

		
		return dec
			
		
	def recEncrypt(self,state,i):
		if i == 10:
			return state
		else:
			subBytes = self.check_cell(self.SubstitutionBytes(state))
			shiftRows = self.check_cell(self.shift_rows(subBytes))
			mixCol = self.check_cell(self.mixColumns(shiftRows))
			roundKey = self.check_cell(self.addRoundKey(mixCol,self.keys[i]))
			return self.recEncrypt(roundKey,i+1)
	
	def recDecrypt(self,state,i):
		if i == 10:
			return state
		else:
			roundKey = self.check_cell(self.addRoundKey(state,self.decKeys[i]))
			mixCol = self.check_cell(self.inv_mix_columns(roundKey))
			shiftRows = self.check_cell(self.inv_shift_rows(mixCol))
			subBytes = self.check_cell(self.inv_SubstitutionBytes(shiftRows))
			return self.recEncrypt(subBytes,i+1)